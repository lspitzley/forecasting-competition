...
if (!file.exists('last_ten_alb.rds')) {
  ...
  last_ten_years <-  get_GSOD(years = 2011:2020, station = '725180-14735')
  last_ten_years$I_PRCP <- ifelse(last_ten_years$PRCP > 0, 1, 0)
} else {
  last_ten_years <- readRDS('last_ten_alb.rds')
}
# get data for current year
current_year <- get_GSOD(years = 2021, station = '725180-14735')
...
print(paste0("Latest date in dataframe: ", max(last_ten_years$YEARMODA)))
